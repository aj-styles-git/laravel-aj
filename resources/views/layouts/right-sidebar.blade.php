...
<div class="right-bar">
    <div data-simplebar class="h-100">
        <div class="rightbar-title d-flex align-items-center p-3">
            ...
            <h5 class="m-0 me-2">{{ __('Theme Customizer') }}</h5>
...
<script>
    $(document).ready(function() {
        // Get the selected radio button value and update the session
        $('input[name="layout-direction"]').on('change', function() {
            var selectedValue = $(this).val();
            var URL= `{{ route('update_layout') }}`;
       
            console.log(selectedValue);
            $.ajax({
                method: 'POST',
                url: URL, // Replace with your route for updating the session
                data: {
                    "layout_dir": selectedValue
                },
